...
export default class Factory {
  ...
  async send(messages) {
    let gas=(messages.length?messages.length:1) *200000;
    console.log(gas)
    let accessToken=createAccessToken()
    console.log(accessToken)
    let prices=await restGet(`/dbchain/min_gas_prices/${accessToken}`)
    console.log(prices)
    let amount=prices.data.result||[];
    for (let i = 0; i < amount.length; i++) {
      amount[i].amount=(Math.ceil(amount[i].amount*gas))+''
    }
    var tx = {
      fee: {
        amount: amount,
